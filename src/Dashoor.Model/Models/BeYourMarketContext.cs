...
using Dashoor.Model.Models.Mapping;
using Dashoor.Core.Migrations;
using Dashoor.Core;

namespace Dashoor.Model.Models
{
    public partial class DashoorContext : Repository.Pattern.Ef6.DataContext
    {
        static DashoorContext()
        {
            ...
            if (DashoorConfigurationManager.MigrateDatabaseToLatestVersion){
                Database.SetInitializer(new MigrateDatabaseToLatestVersion<DashoorContext, ConfigurationInstall<Dashoor.Model.Models.DashoorContext>>());
            }
            else {
                Database.SetInitializer<DashoorContext>(null);
            }
        }

        public DashoorContext()
