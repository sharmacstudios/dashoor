...
namespace Dashoor.Web.Controllers
{
    [Authorize]
    public class ListingController : Controller
    {
        ...
        public async Task<ActionResult> ListingUpdate(Listing listing, FormCollection form, IEnumerable<HttpPostedFileBase> files)
        {
            ...
            if (!User.Identity.IsAuthenticated)
            {
                var accountController = Dashoor.Core.ContainerManager.GetConfiguredContainer().Resolve<AccountController>();
            ...
            foreach (var metaCategory in customFieldCategories)
            {
                ...
                var controlType = (Dashoor.Model.Enum.Enum_MetaFieldControlType)field.ControlTypeID;
